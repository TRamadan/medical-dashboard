...
export class ReportsConfigurationComponent implements OnInit {
  ...
  readonly columns: Array<{ label: string; field: string; type?: 'text' | 'image' | 'date' | 'boolean' | 'custom' | 'number'; sortable?: boolean; filterable?: boolean; filterType?: 'text' | 'date' | 'numeric' | 'boolean'; width?: string; customTemplate?: any; }> = [
    { label: 'Report Name', field: 'name', type: 'text', sortable: true },
  ];

  readonly data = signal([
    {
      id: 1, name: 'Hip Abduction	', createdAt: new Date(), reportTemplate: { id: 1, name: 'Balance' }, reportTemplateDetails: [{ id: 1, name: 'Limb' },
      { id: 2, name: 'Score' },
      { id: 3, name: 'Level' },
    ...
    {
      id: 2, name: 'Hip adduction	', createdAt: new Date(), reportTemplate: { id: 2, name: 'Adductor strength' }, reportTemplateDetails: [
  ...
  updateSelectedReportConfig(): void {
    ...
    this.data.set(
      this.data().map(r => r.id === this.editingReportId ? updated : r)
    );
    this.isEdit = false;
  ...
  onToggleColumn(categoryId: number, columnId: number, columnName: string) {
    ...
    const index = selected.findIndex(
      c => c.categoryId === categoryId && c.columnId === columnId
    );
    let newSelected;
    if (index > -1) {
      // Deselect
    ...
    this.reportConfigForm.get('reportColumns')?.setValue(
      newSelected.map(c => c.columnName).join(', ')
    );
  }
  ...
  onEdit(report: any) {
    ...
    if (report.reportColumns) {
      const selectedNames = (typeof report.reportColumns === 'string')
        ? report.reportColumns.split(',').map((s: string) => s.trim())
        : [];
