...
export class ContactUsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<BodyPart[]>('assets/muscle-polygons (2).json').subscribe(parts => {
  ...
  getComponentProperties(component: WebsiteComponent): ComponentProperty[] {
    const propertyMap: { [key: string]: ComponentProperty[] } = {
      ...
      navbar: [
        { key: 'links', label: 'Navigation Links', type: 'text' }
      ],
      footer: [
      ...
      case 'gallery':
        ...
        <div style="${styles}">
          <div class="gallery-grid">
            ${Array(4).fill(0).map(() => '<div class="gallery-item"><img src="assets/placeholder-image.jpg" alt="Gallery Image" /></div>').join('')}
          </div>
        </div>`;
